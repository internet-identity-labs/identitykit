...
([targets](installation#wrap-provider)) and others (non-targets i.e. ledger
canisters).
...
IdentityKit comes bundled with a [hook](../hooks/useAgent) to get the agent for making authenticated calls
to target and non target canisters, and manages whether or not to show approval pop-ups. You should also
set up an unauthenticatedAgent for calls the user doesn't need to execute themselves.
...
Read more about the [Delegation Toolkit](/miscellaneous/delegation-toolkit) and [differences between accounts and delegations](/miscellaneous/auth-options).
