...
Accounts are wallet addresses. You may query other smart contract canisters for balances 
and other data associated with the account, and request user approval via wallet approval 
prompts to execute transactions.

Delegations are accounts with the authority to execute transactions on the user's behalf,
resulting in the removal of wallet approval prompts, and come in two forms:
...
2. Relying Party Delegations: Accounts unique to your application's URL (never to be used 
for any other application on the network) with the authority to execute transactions
for any smart contract canister on the network.
...
canister IDs that follow [ICRC-28](https://github.com/dfinity/wg-identity-authentication/blob/main/topics/icrc_28_trusted_origins.md). 
In particular, that means each canister listed as a target must
