...
Add the `authType={IdentityKitAuthType.DELEGATION}` prop and populate the `targets=""` prop with a
list of canister IDs and follow the guide to
[remove wallet approval pop-ups](/guides/removing-wallet-pop-ups).

```typescript
const App = () => {
  return (
    <IdentityKitProvider authType={IdentityKitAuthType.DELEGATION} targets="" {...etc}>
      {/* Your App */}
    </IdentityKitProvider>
  );
};
