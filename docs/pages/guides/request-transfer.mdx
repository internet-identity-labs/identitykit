...
The below will display a wallet approval prompt if the user connected with an Account Delegation or Account.

If the user connected with a Relying Party Delegation, your application can already call the ledger on the user's behalf.

```
// signerAgent will know which is the currently connected wallet
const { signerAgent } = useIdentityKit()

// the connected wallet's principal
const connectedPrincipal = SOME_PRINCIPAL

// create an agent for the currently connected wallet
const agent = new signerAgent({
  getPrincipal: () => Principal.fromText(connectedPrincipal),
})
