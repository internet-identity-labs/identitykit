...
Import `IdentityKitAuthType`, add `authOption={IdentityKitAuthType.DELEGATION}`, and your list of `targets` to `IdentityKitProvider`.
...
const App = () => {
  return (
    <IdentityKitProvider
      authOption={IdentityKitAuthType.DELEGATION}
