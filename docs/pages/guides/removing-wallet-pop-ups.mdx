...
fn icrc28_trusted_origins() -> Icrc28TrustedOriginsResponse {
    //you can find an example implementation in the signer-backend package src/cert_trusted.rs
    //certificate query documentation:
    //https://internetcomputer.org/docs/current/developer-docs/smart-contracts/call/overview#certified-queries
...
Import `IdentityKitAuthType`, add `authType={IdentityKitAuthType.DELEGATION}`, and your list of
...
if IdentityKit.connectedWallet.isDelegation {
  if IdentityKit.connectedWallet.delegation.hasTargets {
    ...
    // Use the agent from useIdentityKit hook when making calls to other canisters
  ...
  // Use the agent from useIdentityKit hook when making calls to any canister
}
