---
title: "Local Playground"
date: "2025-01-22"
authors:
  - name: "Dmytro Sheremet"
---

# Local Playground

IdentityKit comes with its own playground to experiment with the features locally.

## Running the Playground

To set up and start the playground, execute the following command:

```sh
npm i && npm run playground
```

## Applications Included

1. **`Dapp` (react-dapp)**  
   Built for signer developers, this application provides a web-based interface to visually explore the API documentation for IdentityKit standards.  
   - **Port:** `3001`  
   - **Dev Environment:** [https://dev.standards.identitykit.xyz/](https://dev.standards.identitykit.xyz/)  
   - **Prod Environment:** [https://standards.identitykit.xyz/](https://standards.identitykit.xyz/)

2. **`Demo` (react-demo)**  
   Designed for application developers, this is a simple frontend application integrated with IdentityKit. It allows users to sign in with IdentityKit and make canister calls.  
   - **Port:** `3002`  
   - **Dev Environment:** [https://dev.demo.identitykit.xyz/](https://dev.demo.identitykit.xyz/)  
   - **Prod Environment:** [https://demo.identitykit.xyz/](https://demo.identitykit.xyz/)

3. **`Signer` (react-signer)**  
   A sample signer application showcasing the basics of signer functionality. It provides insight into the operations on the signer side.  
   - **Port:** `3003`  
   - **Dev Environment:** [https://icrc-25-example-signer-dev.nfid.one/](https://icrc-25-example-signer-dev.nfid.one/)  
   - **Prod Environment:** [https://icrc-25-example-signer.nfid.one/](https://icrc-25-example-signer.nfid.one/)
